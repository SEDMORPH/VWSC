; $Id: //depot/idl/IDL_703/idldir/lib/pnt_line.pro#1 $
;
; Copyright (c) 1993-2008, ITT Visual Information Solutions. All
;       rights reserved. Unauthorized reproduction is prohibited.
;

;; VW altered to tell you if -tve or +tve distance
Function vw_Pnt_Line, p0, l0, l1, pl, INTERVAL = interval
...
if l eq 0 then begin	;Line is a point
   pl = p0
   ans = sqrt(total((l0-float(p0))^2))
   if p0 lt pl then ans=-ans 
   return, ans
endif
...
if keyword_set(interval) and out then begin  ;Outside interval?
    ...
    ans = d1 < d2
    if p0[0] lt pl[0] then ans=-ans
    return, ans
endif else begin
   ans = sqrt(total((p0 - pl)^2))
    if p0[0] lt pl[0] then ans=-ans
   return, ans
endelse
